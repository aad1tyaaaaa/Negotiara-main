...
def execute_negotiation(context: dict, max_rounds: int = 5) -> dict:
    """
    Full Simulation Loop (unchanged behavior, refactored implementation).
    """
    ...
    for current_round in range(1, max_rounds + 1):
        ...
        history.append({"role": "SHIPPER", "content": shipper_response.get("message_to_lsp"), "price": current_shipper_price})
        ...
        carrier_min = c_metrics.get("operational_cost", adjusted_intrinsic * 0.95)
        ...
        history.append({"role": "CARRIER", "content": carrier_response.get("message_to_lsp"), "price": current_carrier_price})
    ...
    return {
        ...
        "market_context": {"intrinsic_value": adjusted_intrinsic, "market_benchmark": market_avg},
        "rounds": rounds_data,
        "history": history
    }
...
def execute_single_step(context: dict, history: list, role: str = "SHIPPER") -> dict:
    ...
    reservation_price = context.get(metrics_key, {}).get("budget" if role == "SHIPPER" else "operational_cost", market_avg)
    ...
    agent_context = {
        ...
        "intrinsic_value": adjusted_intrinsic,
        "strategy_profile": prep["strategy_profile"],
        "target_price": context.get(metrics_key, {}).get("initial_offer", market_avg),
